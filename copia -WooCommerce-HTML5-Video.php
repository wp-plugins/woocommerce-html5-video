...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
  ...
  function register_my_setting() {
   register_setting( 'dimensions_group', 'width', 'intval' );
   register_setting( 'dimensions_group', 'height', 'intval' );
  }
...
function my_plugin_menu() {
   add_options_page( 'Html5 Video Settings', 'Woocommerce html5 video', 'manage_options', 'html5-video-settings', 'my_plugin_options' );
}
...
function my_plugin_options() {
   ...
   $content = '<div class="wrap">'.screen_icon().'<h2>Woocommerce Html5 Video Settings</h2>
   <form method="post" action="options.php">'.
   settings_fields( 'dimensions_group' ).
   do_settings_fields( 'dimensions_group' ).
   '<span>Configure the video dimensions.</span>
   <table class="form-table">
        <tr valign="top">
        <th scope="row">Video Width:</th>
        <td><input type="text" name="new_option_name" value="'.get_option('width').'" /></td>
        </tr>
        <tr valign="top">
        <th scope="row">Video Height:</th>
        <td><input type="text" name="option_etc" value="'.get_option('height').'" /></td>
        </tr>
    </table>'.
